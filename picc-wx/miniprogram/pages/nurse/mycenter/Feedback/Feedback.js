// pages/nurse/mycenter/Feedback/Feedback.js
...
Page({
  ...
  submit() {
    if (!this.data.problem) {
      ...
      let params = {
        ...
        username: this.data.nurseInfo.username
      };
        ...
        if (res.status == 200) {
          wx.showToast({
            ...
            success(data) {
              setTimeout(function () {
                wx.redirectTo({
                  url: '/pages/nurse/mycenter/FeedbackSuccess/FeedbackSuccess',
                })
              }, 1000) //延迟时间
            }
          })
  ...
  historyJump() {
    wx.navigateTo({
      url: '/pages/nurse/mycenter/FeedbackHistory/FeedbackHistory',
    })
  },
  /**
   * 生命周期函数--监听页面加载
