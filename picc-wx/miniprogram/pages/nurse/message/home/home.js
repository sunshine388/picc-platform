  ...
  methods: {
    async init() {
      this.try(async () => {
        ...
        const _ = db.command;
        const myNumber=wx.getStorageSync('nurseInfo').jobnumber;
    ...
    messageItemJump(e) {
      ...
      const patient = {
        patientname: this.data.chatList[e.currentTarget.dataset.index].name,
        patientnumber: this.data.chatList[e.currentTarget.dataset.index].otherNumber,
      }
      wx.navigateTo({
        url: '/pages/nurse/message/messageitem/messageitem?patientInfo='+JSON.stringify(patient),
      })
    },
    // ListTouch触摸开始
    ListTouchStart(e) {
